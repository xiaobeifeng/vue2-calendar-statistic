 ...
 *
 * @version 2.0.17
 * 增加bdocr，bdocrnoqc相关方法
 *
 * @version 2.0.16
 * 增加wxpay，相关方法
 *
 * @version 2.0.15
 * 增加（易联众医保支付）ecpay，相关方法
 *
 * @version 2.0.14
 * 修正showPdfWithVid函数兼容性
 *
 * @version 2.0.13
 * 从nanchang的location合入requestAlipayVerify函数，变更为通用函数
 *
 * @version 2.0.12
 * 增加requestWxLaunchMiniProgram函数
 *
 * @version 2.0.11
 * 修改openPolyvPlayer函数，增加参数isSpeedShow，callbackGap
 *
 * @version 2.0.10
...
;(function () {
   ...
   */
  var viewPdfCallBack = function (val) {}
  function viewPdfCallbackSuccessed(val) {
    viewPdfCallBack(val)
  }
  function viewPdf(paramJsonObj, callback) {
    viewPdfCallBack = callback
    if (window.iosdkqgqc) {
   ...
   * callback 存储结果回调
   */
  function saveAccountInfoWithCallback(
    accountId,
    token,
    refreshToken,
    userName,
  ...
  // 微信支付
  /**
   * @description 定义的微信支付回调
   * res为json对象:{resultStatus:"状态码"} 9000：支付成功，8000：支付Pending，6001：支付取消，其他：支付失败
   * @ignore
   */
  var requestWxPayCallback = new function (res) {}()
   ...
   * @param {*} obj: {payParam:"支付参数"}
   */
  function requestWxPay(obj, callback) {
    requestWxPayCallback = callback
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.requestWxPay.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.requestWxPay',
        obj,
  ...
  function requestWxPayCallbackSuccessed(res) {
    console.log(res)
    requestWxPayCallback(res)
  }
  // 微信支付结束
   ...
   * @param {*} obj: {vid:"vid"f,title:"title",viewerId:"viewerId",continuePos:66000,isplayed:true,isSpeedShow:false,callbackGap:10}
   * 特别注意continuePos为整数形，代表续播的时候跳到的指定的毫秒数;callbackGap为整数，代表进度回调的时间间隔的秒数
   * @param {*} callback:进度回调
  ...
  /**
   * @description 微信分享
   * @param{*}  obj: {scene:"1"f,title:"标题",thumbUrl:"分享的图标的Base64",desc:"描述",link:"分享链接"}
   * 参数说明：
  ...
  // -------------------weixin miniProgram begin---------------------------------------

  /**
   * @description 从微信返回时的回调，不保证一定会调用此函数
   * @param {*} val 为{error:'',extMsg:'11111'}
   * 如果成功error字段为空，其他情况下error为错误信息
   * extMsg为小程序返回的值
   * @ignore
   */
  var requestWxLaunchMiniProgramCallBack = function (val) {}

  /**
   * @description 微信miniProgram
   * @param{*}  obj: {userName:"拉起的小程序的username(小程序原始id)",path:"拉起小程序页面的带参路径",miniProgramType:"RELEASE/PREVIEW/TEST"}
   * 参数说明：
   * userName：string类型，拉起的小程序的username(小程序原始id)
   * path：string类型，拉起小程序页面的带参路径
   * miniProgramType：string类型，RELEASE/PREVIEW/TEST
   */
  function requestWxLaunchMiniProgram(obj, callBack) {
    requestWxLaunchMiniProgramCallBack = callBack

    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.requestWxLaunchMiniProgram.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.requestWxLaunchMiniProgram',
        obj,
        function (responseData) {}
      )
    } else if (window.WebViewJavascriptBridge && !window.AlipayJSBridge) {
  ...
  function onRequestWxLaunchMiniProgramCallBack(val) {
    requestWxLaunchMiniProgramCallBack(val)
  }

  // ------------------weixin miniProgram end------------------------------------------
   ...
   * obj为json对象
   */
  var requestAlipayVerifyCallback = function (obj) {}

  /**
   * @description alipay verify
   * obj:调用参数
   * obj包含如下
   * {url:"alipay的verify url",certifyId:"alipay的certifyId"}
   */
  function requestAlipayVerify(obj, invokeCallback) {
    if (invokeCallback != null) requestAlipayVerifyCallback = invokeCallback

    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.requestAlipayVerify.postMessage({
        body: obj
      })
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.requestAlipayVerify',
        obj,
        function (responseData) {}
      )
    } else if (window.WebViewJavascriptBridge && !window.AlipayJSBridge) {
  ...
  /**
   * @description 获取alipay verify的结果
   * @param callbackparam 具体格式请参照alipay verify的sdk的相关说明
   * @ignore
   */
  function requestAlipayVerifySuccessed(callbackparam) {
    requestAlipayVerifyCallback(callbackparam)
  }
  ...
  //* **************ecpayOpenSettleList begin*************************************
  /**
   * @description ecpayOpenSettleList 待结算列表查询
   * @param {*} obj:{ecAuthCode:'',ecChnlUserId:'',ecChnlAppId:''}
   *
   */
  function ecpayOpenSettleList(obj) {
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.ecpayOpenSettleList.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.ecpayOpenSettleList',
        obj,
        function (responseData) {}
      )
    } else if (window.WebViewJavascriptBridge && !window.AlipayJSBridge) {
  ...
  //* **************ecpayOpenSettleList end***************************************

  //* **************ecpayOpenSettleRecord begin*************************************
  /**
   * @description ecpayOpenSettleRecord 结算记录查询
   * @param {*} obj:{ecAuthCode:'',ecChnlUserId:'',ecChnlAppId:''}
   *
   */
  function ecpayOpenSettleRecord(obj) {
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.ecpayOpenSettleRecord.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.ecpayOpenSettleRecord',
        obj,
        function (responseData) {}
      )
    } else if (window.WebViewJavascriptBridge && !window.AlipayJSBridge) {
  ...
  //* **************ecpayOpenSettleRecord end***************************************

  //* **************ecpayOpenBankManager begin*************************************
  /**
   * @description ecpayOpenBankManager 银行卡管理
   * @param {*} obj:{ecAuthCode:'',ecChnlUserId:'',ecChnlAppId:''}
   *
   */
  function ecpayOpenBankManager(obj) {
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.ecpayOpenBankManager.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.ecpayOpenBankManager',
        obj,
        function (responseData) {}
      )
    } else if (window.WebViewJavascriptBridge && !window.AlipayJSBridge) {
  ...
  //* **************ecpayOpenBankManager end***************************************

  //* **************ecpayDealMessage begin*************************************
  //  /**
  //  * @description ecpayDealMessage 消息处理
  //  * @param {*} obj:{ecAuthCode:'',ecChnlUserId:'',ecChnlAppId:'',message:''}
  //  *
  //  */
  //  function ecpayDealMessage(obj) {

  //    if(window.iosdkqgqc) {
  //       window.webkit.messageHandlers.ecpayDealMessage.postMessage(obj);
  //    }
  //    else if(window.WebViewJ2CJavascriptBridge) {
  //        window.WebViewJ2CJavascriptBridge.callHandler(
  //            'Native.ecpayDealMessage', obj,
  //            function(responseData) {}
  //        );
  //    }
  //    else if(window.WebViewJavascriptBridge && !window.AlipayJSBridge) {
  ...
  /**
   * @description 定义wxpay的回调
   * res为json对象:{errorCode:"状态码"} 0：支付成功，-2：支付取消，其他：支付失败
   * @ignore
   */
  var requestWxpayCallback = new function (res) {}()
   ...
   * @description wxpay
   * @param {*} callback:  格式为function(res) {}
   * @param {*} obj: {payParam:支付参数的json字符串},注意:payParam是key，其值为json字符串
   * payParam的json字符串内容包括：
   * partnerId(值为字符串)，prepayId(值为字符串)，package(值为字符串)，nonceStr(值为字符串)，timeStamp(值为字符串)，sign(值为字符串)
   */
  function requestWxpay(obj, callback) {
    requestWxpayCallback = callback
    if (window.iosdkqgqc) {
  ...
  /**
   * @description 获取当前wxpay的callback的Native回调
   * res为json对象:{errorCode:"状态码"} 0：支付成功，-2：支付取消，其他：支付失败
   * @ignore
   */
  function requestWxpayCallbackSuccessed(res) {
   ...
   *   callbackOnShow：实现为无参函数callbackOnShow（），可以在此函数回调时关闭js前端等待的蒙版
   *   callbackOnComplete：实现为有参函数callbackOnComplete（val），
   *   其中val的格式为json对象，固定会有一个error的key，如果发生错误其中会是错误信息，如果没有错误error的值为空,error为cancel时为用户取消
   *   成功时val中会有side(side:0人像，1国徽)，idcardImg（图像base64字符串）
   */
  function openBdOcr(obj, callbackOnShow, callbackOnComplete) {
    var serialNumOnShow = uuid_kit()
    var serialNumOnComplete = uuid_kit()
    ...
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.openBdOcr.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.openBdOcr',
        obj,
   ...
   *
   */
  function onCompleteOpenBdOcr(par) {
    var serialNum = par.serialNum
    var val = par.val
    for (x in callArray_kit) {
      var s = callArray_kit[x].serialNum
  ...
  // ----------------baidu ocr without quality control begin--------------------

  /**
   *  @description 调用ocr获取图像(没有QC)
   *   obj：配置参数，一维json对象(side:0人像，1国徽)
   *   callbackOnShow：实现为无参函数callbackOnShow（），可以在此函数回调时关闭js前端等待的蒙版
   *   callbackOnComplete：实现为有参函数callbackOnComplete（val），
   *   其中val的格式为json对象，固定会有一个error的key，如果发生错误其中会是错误信息，如果没有错误error的值为空，error为cancel时为用户取消
   *   成功时val中会有side(side:0人像，1国徽)，idcardImg（图像base64字符串）
   */
  function openBdOcrNoQc(obj, callbackOnShow, callbackOnComplete) {
    var serialNumOnShow = uuid_kit()
    var serialNumOnComplete = uuid_kit()
    ...
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.openBdOcrNoQc.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.openBdOcrNoQc',
        obj,
        function (responseData) {}
      )
    } else if (window.WebViewJavascriptBridge && !window.AlipayJSBridge) {
  ...
  /**
   *  @description Ocr检测onShow回调（没有QC）
   *  @ignore
   */
  function onShowOpenBdOcrNoQc(par) {
    var serialNum = par.serialNum
    for (x in callArray_kit) {
      var s = callArray_kit[x].serialNum
      var callback = callArray_kit[x].callback
  ...
  /**
   *  @description Ocr检测onComplete回调(没有QC)
   *  @ignore
   *  par为json对象，serialNum为字符串，val为Json字符串，里面是传递的信息
   *  val里固定会有error字段，如果成功error字段为空，其他情况下error为错误信息
   *
   */
  function onCompleteOpenBdOcrNoQc(par) {
    var serialNum = par.serialNum
    var val = par.val
    for (x in callArray_kit) {
      var s = callArray_kit[x].serialNum
  ...
  // ----------------baidu ocr without quality control end--------------------
   ...
   * 强制关闭 app
   */
  function forceQuit(obj) {
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.forceQuit.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.forceQuit',
        obj,
   ...
   * 清空老版本系统缓存问题 app
   */
  function cleanOldAPPSP() {
    if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.cleanOldAPPSP',
        {},
        function (responseData) {}
      )
    } else {
  ...
  // 旷视活体检测 sdk 调用开始
  var rexamineLivingCallback = new function (res) {}()

  /**
   * @description 旷视活体检测 sdk 调用
   * @param {*} callback:  格式为function(res) {}
   * @param {*} obj: {}
   */
  function examineLiving(obj, callback) {
    rexamineLivingCallback = callback
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.examineLiving.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.examineLiving',
        obj,
        function (responseData) {}
      )
    } else if (window.WebViewJavascriptBridge && !window.AlipayJSBridge) {
  ...
  function examineLivingCallbackSuccessed(res) {
    console.log(res)
    rexamineLivingCallback(res)
  }
  // 旷视活体检测 sdk 调用结束

  // 旷视 ocr sdk 调用开始
  var examineIDCardCallback = new function (res) {}()

  /**
   * @description 旷视 ocr sdk 调用
   * @param {*} callback:  格式为function(res) {}
   * @param {*} obj: {}
   */
  function examineIDCard(obj, callback) {
    examineIDCardCallback = callback
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.examineIDCard.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.examineIDCard',
        obj,
        function (responseData) {}
      )
    } else if (window.WebViewJavascriptBridge && !window.AlipayJSBridge) {
  ...
  function examineIDCardCallbackSuccessed(res) {
    console.log(res)
    examineIDCardCallback(res)
  }
  // 旷视 ocr sdk 调用结束

  // 安卓下载 apk
  function downloadApk(obj) {
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.downloadApk.postMessage(obj)
    } else if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.downloadApk',
        obj,
  ...
  // 申请权限（包含网络定位和gps定位及其他权限） 调用开始
  var accessPermissionsCallback = new function (res) {}()

  /**
   * @description 申请权限（包含网络定位和gps定位及其他权限）
   * @param {*} callback:  格式为function(res) {}
   * @param {*} obj: {}
   */
  function accessPermissions(obj, callback) {
    accessPermissionsCallback = callback
    if (window.WebViewJ2CJavascriptBridge) {
      window.WebViewJ2CJavascriptBridge.callHandler(
        'Native.accessPermissions',
        obj,
        function (responseData) {}
      )
    }
  ...
  function accessPermissionsCallbackSuccessed(res) {
    console.log(res)
    accessPermissionsCallback(res)
  }
  // 申请权限（包含网络定位和gps定位及其他权限） 调用结束

  var requestAppInfoCallBack = function (val) {}

  function requestAppInfo(obj, callBack) {
    requestAppInfoCallBack = callBack
    if (window.iosdkqgqc) {
      window.webkit.messageHandlers.appInfo.postMessage(obj)
    } else {
      throw new Error('J2C env init error')
    }
  }

  function onGetAppVersionCodeSuccessed(val) {
    requestAppInfoCallBack(val)
  }
  ...
  window['J2C']['requestPensionCertificationSuccessed'] =
    requestPensionCertificationSuccessed
  window['J2C']['requestInjuryCertification'] = requestInjuryCertification
  window['J2C']['requestInjuryCertificationSuccessed'] =
    requestInjuryCertificationSuccessed
  ...
  window['J2C']['viewPdfCallbackSuccessed'] = viewPdfCallbackSuccessed
  ...
  window['J2C']['requestAlipayCallbackSuccessed'] =
    requestAlipayCallbackSuccessed

  window['J2C']['requestWxPay'] = requestWxPay
  window['J2C']['requestWxPayCallbackSuccessed'] = requestWxPayCallbackSuccessed

  window['J2C']['requestLngAndLat'] = requestLngAndLat
  window['J2C']['requestLngAndLatCallbackSuccessed'] =
    requestLngAndLatCallbackSuccessed
  ...
  window['J2C']['requestWxLaunchMiniProgram'] = requestWxLaunchMiniProgram
  window['J2C']['onRequestWxLaunchMiniProgramCallBack'] =
    onRequestWxLaunchMiniProgramCallBack

  window['J2C']['requestAlipayVerify'] = requestAlipayVerify
  window['J2C']['requestAlipayVerifySuccessed'] = requestAlipayVerifySuccessed

  window['J2C']['ecpayOpenSettleList'] = ecpayOpenSettleList
  window['J2C']['ecpayOpenSettleRecord'] = ecpayOpenSettleRecord
  window['J2C']['ecpayOpenBankManager'] = ecpayOpenBankManager
  // window['J2C']['ecpayDealMessage'] = ecpayDealMessage;

  window['J2C']['requestWxpay'] = requestWxpay
  window['J2C']['requestWxpayCallbackSuccessed'] = requestWxpayCallbackSuccessed

  window['J2C']['openBdOcr'] = openBdOcr
  window['J2C']['onShowOpenBdOcr'] = onShowOpenBdOcr
  window['J2C']['onCompleteOpenBdOcr'] = onCompleteOpenBdOcr

  window['J2C']['openBdOcrNoQc'] = openBdOcrNoQc
  window['J2C']['onShowOpenBdOcrNoQc'] = onShowOpenBdOcrNoQc
  window['J2C']['onCompleteOpenBdOcrNoQc'] = onCompleteOpenBdOcrNoQc

  window['J2C']['forceQuit'] = forceQuit

  window['J2C']['cleanOldAPPSP'] = cleanOldAPPSP

  window['J2C']['examineLiving'] = examineLiving
  window['J2C']['examineLivingCallbackSuccessed'] =
    examineLivingCallbackSuccessed

  window['J2C']['examineIDCard'] = examineIDCard
  window['J2C']['examineIDCardCallbackSuccessed'] =
    examineIDCardCallbackSuccessed

  window['J2C']['downloadApk'] = downloadApk

  window['J2C']['accessPermissions'] = accessPermissions
  window['J2C']['accessPermissionsCallbackSuccessed'] =
    accessPermissionsCallbackSuccessed

  window['J2C']['appInfo'] = requestAppInfo
  window['J2C']['onGetAppVersionCodeSuccessed'] = onGetAppVersionCodeSuccessed
